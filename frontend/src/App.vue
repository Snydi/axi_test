...
const activePage = ref(window.location.hash === '#applications' ? 'applications' : 'clients')
...
function openPage(pageName) {
  ...
  window.location.hash = pageName === 'applications' ? 'applications' : 'clients'
}
...
function handleHashChange() {
  activePage.value = window.location.hash === '#applications' ? 'applications' : 'clients'
}
...
onMounted(() => {
  ...
  activePage.value === 'applications' ? loadApplications(0) : loadClients(0)
})
    ...
    <template v-else>
      <header class="page-header">
        <div>
          ...
          <p class="subtitle">Все заявки и актуальный статус решения</p>
        </div>
      ...
      <section class="panel" aria-labelledby="applications-heading">
        <div class="toolbar">
          <h2 id="applications-heading">Список заявок</h2>
        </div>
